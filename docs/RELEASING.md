...
```bash
git tag -a v1.4.7 -m "Quest APK Renamer 1.4.7"
git push origin v1.4.7
```

The release workflows build:

- `Quest-APK-Renamer-1.4.7-Windows-portable.zip`;
- `Quest-APK-Renamer-1.4.7-Setup.exe`;
- `Quest-APK-Renamer-1.4.7-macOS-arm64.dmg`;
- `Quest-APK-Renamer-1.4.7-macOS-x86_64.dmg`;
- `Quest-APK-Renamer-1.4.7-Linux-x86_64.tar.gz`; and
- `Quest-APK-Renamer-1.4.7-x86_64.AppImage`.
...
version exactly or add a prerelease suffix. For example, app version `1.4.7`
accepts `v1.4.7` or `v1.4.7-beta.1`. Tags containing a hyphen are marked as
prereleases. Create the stable `v1.4.7` tag only after the candidate Windows,
